...
use tulpje_framework::{
    ...
    handler_func, Error, Framework, ModuleBuilder, Registry,
};
...
async fn main() {
    ...
    framework.start().await.expect("error running framework");

    let event = DiscordEvent::new(1, EVENT_JSON.replace("__COMMAND__", "command-name-500"));
    ...
    for iteration in 0..total_iterations {
        ...
        sender
            .send(event.meta.clone(), discord_event.clone())
            .expect("error queueing event");
    }
